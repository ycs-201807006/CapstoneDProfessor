...
namespace Server
{
    ...
    class MyServer
    {
        ...
        private void SendExamStart()
        {
            foreach (var item in ClientServerData)
            {
                NetworkStream ClientStream = item.Value.GetStream();
                ClientStream.Write(new byte[1] { 4 }, 0, 1); // byte=1의 데이터를 보낸다.
            }
        }
        ...
        private void Fliesendfromserver(NetworkStream stream, string targetclient) // 서버에서 targetclient에게 파일을 보낼때 사용
        {
        ...
        private void allSendFile() // 현재 접속해 있는 모든 유저에게 파일 전송
        {
            ZipManager.ZipFiles(@"D:\ZipTest\unzipFolder\", @"D:\ZipTest1.zip", "1234", false);

            NetworkStream stream;

            FileInfo file = new FileInfo("D:\\ZipTest1.zip");
            ...
            foreach (var display in ClientServerData)
            {
                ...
                foreach (var item in connectedClients)
                {
                    if (display.Key == item.Key)
                    {
                        hak = item.Value.hak; // 보내는 유저의 학번 가져오기
                    }
                }
                ...
                using (FileStream filestream = new FileStream(file.FullName, FileMode.Open, FileAccess.Read))
                {
                    ...
                    Action<byte[]> Send = (b) =>
                    {
                        byte[] datasize = BitConverter.GetBytes(b.Length);
                        // 먼저 데이터 사이즈를 보내고.
                    ...
                    if (ret[0] == 1)
                    {
                        Console.WriteLine(hak + "에게 전송 완료");
                    }
                }
            }
        }
